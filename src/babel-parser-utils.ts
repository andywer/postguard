...
function isRequireCall(path: NodePath<types.CallExpression>) {
  ...
  return (
    callee.isIdentifier() && callee.node.name === "require" && !callee.scope.getBinding("require")
  )
}
...
export function getReferencedNamedImport(
  ...
  if (
    ...
    bindingPath.get("init").isMemberExpression()
  ) {
    ...
    if (
      id.node.name === binding.identifier.name &&
      initObject.isCallExpression() &&
      isRequireCall(initObject)
    ) {
      if (
